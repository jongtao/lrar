...
#define TX_FCTRL_MASK_32		0xFFFFE3FFUL	/* access mask to TX_FCTRL_ID */
/*masks (low 32 bit) */
...
class DW1000 {
	public:
		...
		void resetChip();
		void softReset();
		void printAnything(byte cmd, int subindex, uint16_t offset, int n);
		void printAnythingln(byte cmd, int subindex, uint16_t offset, int n);
		
		void writePANAddress();
		void initialise();
		void mySettingsChannel1();
		void loadLDE();

		// read information from registers
		uint64_t readDeviceIdentifier();
		uint64_t readSystemConfiguration();
		uint64_t readSystemStatus();
		uint64_t readICTempAndVoltage();
		double readRxTimestamp();
		...
		int fpsp_n();
		int rx_c();
		double readFirstPathSignalPower(int PRF);		// estimate first path signal power
		double readRxLevel(int PRF);				// estimate receive signal power
		void readOTP(int addr);

		void clearSystemStatus(long bit);
		void setSystemConfig(uint64_t buffer);
		void toggleGPIO_MODE();

		void setTxFrameControl(long buffer);
		void writeTxBuffer(byte buffer[], int n);
		uint64_t readRxBuffer(uint16_t offset, int n);
		void startTx();
		void startRx();
		void startTxRx();
		void stopTxRx();
        void setPanID(uint16_t val);
        void setShortID(uint16_t val);
        void writeReg(byte cmd, int subindex, uint16_t offset, uint64_t buffer, int n);
        uint64_t readReg(byte cmd, int subindex, uint16_t offset, int n);
        void writeBytes(byte cmd, word offset, byte data[], unsigned int n);
				
	private:
		/* Global internal variables */
		int _ss;								// Slave Select number (53 for Tx, 47 for Rx)
		int _rst;								// Reset pin (pin 9)
		long _syscfg;							// System Config
		long _sysstat;							// System Status
		long _ICtemp;							// IC temperature readings
		long _ICvoltage;						// IC voltage readings
		...
		static const byte SUB = 0x01;			// Sub-indexing is used
		static const byte NO_SUB = 0x00;		// No sub-indexing
		static const byte NO_OFFSET = 0x00;		// No offset
